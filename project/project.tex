...
The classifiers require numerical feature vector of fixed length rather than variable length raw text. Our processed tweets had to be converted into numerical feature vectors. This process of converting the raw text to a numerical feature vector is called vectorization.

Six different feature representation methods were used.

\subsubsection{Bag-of-Words}
One method of vectorization is Bag-of-Words (BOW). The Count Vectorizer from Python's Scikit Learn was used to implement this. In BOW documents are described by word occurrences.

A vocabulary is created of all unique terms in the dataset, ranked in terms of frequency of occurrence. The maximum size of the vocabulary can be specified, so only the top however many terms are kept. Each tweet is then represented as a series of ones and zeroes. Ones representing term occurrences and zeroes representing a lack of occurrence.

A major drawback of Bag-of-Words is that it does not take word ordering into account. This is why it is called 'bag' of words. You can imagine all the words have been thrown into a bag together.

\subsubsection{TF-IDF}

BOW can be extended with TF-IDF (Term Frequency Inverse Document Frequency). It is used to re weight the count features. Term frequency is the frequency of the word in the current document. Inverse Document Frequency takes into account how often the word occurs in the whole dataset. The idea is to balance how important a term is in a document versus how important it is in the entire collection. We are less interested in a very frequently occurring term like for example 'the' than some less frequently occurring words.

%Also, very short texts are likely to have noisy tf–idf values while the binary occurrence info is more stable.

\subsubsection*{N-Grams}

An N-gram is a sequence of n consecutive terms. For example the bigrams of 'Twitter as an Alternative Review Site' are: 'Twitter as', 'as an', 'an Alternative', 'Alternative Review' and 'Review Site'.

If bi-grams are combined with BOW, occurrences of pairs of consecutive words are counted instead of individual terms. This helps to address the problem BOW has with discarding word ordering.

BOW was implemented with unigrams, bigrams and trigrams.
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{project/accuracy_graph.png}
